...
class TaskDatabase(context:Context):SQLiteOpenHelper(context, DATABASE_NAME,null, DATABASE_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        val createTableQuery="CREATE TABLE $TABLE_NAME($COLUMN_ID INTEGER PRIMARY KEY,$COLUMN_TITLE TEXT,$COLUMN_TIME TEXT,$COLUMN_NOTE TEXT,$COLUMN_PRIORITY TEXT)"
    ...
    fun getAllTasks(): List<Task> {
        ...
        while (cursor.moveToNext()) {
            ...
            val task = Task(id, title, time,note, priority,)
            taskList.add(task)
        }
    ...
    fun getTaskById(taskId: Int): Task {
        ...
        cursor.close()
        db.close()
        return Task(id, title, time,note, priority,)
    }
